...
(function ($) {
    ...
    $.fn.imageUploader = function (options) {
        ...
        let defaults = {
            ...
            label: 'Dosyaları buraya sürükleyip bırakın veya eklemek için tıklayın (maksimum 2MB)',
            extensions: ['.jpg', '.jpeg', '.png','.svg','.ico'],
            mimes: ['image/jpeg', 'image/png','image/svg+xml','image/x-icon'],
            maxSize: undefined,
            maxFiles: 5,
        };
        ...
        let validateExtension = function (file) {
            ...
            if (plugin.settings.extensions.indexOf(file.name.replace(new RegExp('^.*\\.'), '.')) < 0) {
                alert(`"${file.name}" dosyası, kabul edilen dosya uzantılarıyla eşleşmiyor: "${plugin.settings.extensions.join('", "')}"`);
        ...
        let validateMIME = function (file) {
            ...
            if (plugin.settings.mimes.indexOf(file.type) < 0) {
                alert(`"${file.name}" dosyası, kabul edilen MIME türleriyle eşleşmiyor: "${plugin.settings.mimes.join('", "')}"`);
        ...
        let validateMaxSize = function (file) {
            ...
            if (file.size > plugin.settings.maxSize) {
                alert(`"${file.name}" dosyası, ${plugin.settings.maxSize / 1024 / 1024}Mb olan maksimum boyutu aşıyor.`);
        ...
        let validateMaxFiles = function (index, file) {
            ...
            if ((index + dataTransfer.items.length + plugin.settings.preloaded.length) >= plugin.settings.maxFiles) {
                alert(`${plugin.settings.maxFiles} dosya sınırına ulaşıldığı için "${file.name}" dosyası eklenemedi`);
